        ...
        for (instance, id) in factory.instances() {
            match &instance {
                Instance::Spawner { name, data, phase_tag, content, .. } => {
                    ...
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::Spawner, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::Npc { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<NpcOtherlandClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::NpcOtherland, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::Structure { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<StructureClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::Structure, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::Portal { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<PortalClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::Portal, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::StartingPoint { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<StartingPointClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::StartingPoint, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::Trigger { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<TriggerClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::Trigger, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::ChessPiece { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<ChessPieceClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::ChessPiece, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::Ship { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<ShipClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::Ship, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::Planet { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<PlanetClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::Planet, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::InteractObject { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<InteractObjectClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::InteractObject, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::PatrolNode { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<PatrolNodeClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::PatrolNode, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::SpawnNode { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<SpawnNodeClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::SpawnNode, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::MinigameInfo { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<MinigameInfoClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::MinigameInfo, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::ChessMetaGameLogic { name, data, phase_tag, content, .. } => {
                    ...
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::ChessMetaGameLogic, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::EDNAContainer { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<EdnaContainerClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::EDNAContainer, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::BilliardBall { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<BilliardBallClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::BilliardBall, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::OtherlandStructure { name, data, phase_tag, content, .. } => {
                    ...
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::OtherlandStructure, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::MinigameScoreBoard { name, data, phase_tag, content, .. } => {
                    ...
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::MinigameScoreBoard, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::PresetPoint { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<PresetPointClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::PresetPoint, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::Door { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<DoorClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::Door, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::CTFGameFlag { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<CtfGameFlagClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::CTFGameFlag, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::ServerGateway { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<ServerGatewayClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::ServerGateway, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::ServerGatewayExitPhase { name, data, phase_tag, content, .. } => {
                    ...
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::ServerGatewayExitPhase, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::NonSpawnPlacement { name, data, phase_tag, content, .. } => {
                    ...
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::NonSpawnPlacement, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::MyLandSettings { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<MyLandSettingsClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::MyLandSettings, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::WorldDisplay { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<WorldDisplayClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::WorldDisplay, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::MypadRoomDoor { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<MypadRoomDoorClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::MypadRoomDoor, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::QuestBeacon { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<QuestBeaconClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::QuestBeacon, name, phase_tag, instance.guid().to_owned(), params);
                },
                Instance::CustomTrigger { name, data, phase_tag, content, .. } => {
                    let mut params = content.to_owned().into_param::<CustomTriggerClass>().unwrap();
                    params.apply(data.to_owned());
                    
                    self.spawn_non_player_avatar(id.to_owned(), EntityType::CustomTrigger, name, phase_tag, instance.guid().to_owned(), params);
                },
            }
        }
