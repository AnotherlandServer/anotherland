...
fn main() {
    ...
    match cli.command {
        ...
        Some(Commands::Export { pepkg_path, output_path }) => {
            env_logger::builder()
                .filter_level(if cli.verbose {
                    log::LevelFilter::Trace
                } else {
            ...
            info!("Exporting {pepkg_path} to {output_path}");
            ...
            if std::path::Path::new(&pepkg_path).exists() {
                ...
                for i in 0..pepkg.tile_count() {
                    ...
                    let tile_x = (mesh.tiled_cp_face_index_translator_2.federation_tile_index % tiles_horizontal) as f32 * federation.tile_size as f32 + mesh.tiled_cp_face_index_translator_2.start_x as f32;
                    let tile_z = (mesh.tiled_cp_face_index_translator_2.federation_tile_index / tiles_horizontal) as f32 * federation.tile_size as f32 + mesh.tiled_cp_face_index_translator_2.start_y as f32;

                    let tile_mesh = load_3d_mesh(&mesh);

                    let prev_count = vertices.len();
                    vertices.extend(
                        tile_mesh.attribute(Mesh::ATTRIBUTE_POSITION).unwrap()
                            .as_float3().unwrap()
                            .iter()
                            .map(|v| Vec3::new(v[2] + tile_z, v[1], -(v[0] + tile_x)))
                    );

                    indices.extend(
                        tile_mesh.indices().unwrap()
                            .iter()
                            .map(|i| (i + prev_count) as u32)
                            .collect::<Vec<_>>()
                            .chunks(3)
                ...
                write_obj("Navmesh", &vertices, &indices, output_path)
...
fn setup_display(
    ...
    for i in 0..pepkg.tile_count() {
        ...
        commands.spawn((
            ...
            Transform::from_translation(Vec3::new(tile_x, i as f32 * -10.0, tile_z)),
        ));

        commands.spawn((
            Mesh3d(meshes.add(Cuboid::new(
                federation.tile_size as f32, 
                400.0, 
                federation.tile_size as f32
            ))),
            Wireframe,
