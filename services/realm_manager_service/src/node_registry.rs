...
struct NodeRegistryData {
    ...
    core: Arc<CoreClient>,
    nodes: HashMap<PeerIdentity, (Uuid, NodeType, String)>,
}
...
impl NodeRegistry {
    ...
    pub async fn register_node(&self, peer_identity: PeerIdentity, node_type: NodeType, url: String) {
        let mut state = self.0.write().await;
        let realm_server = state.realm_server.clone();
        let core = state.core.clone();

        let entry = state.nodes.entry(peer_identity)
            .or_insert((Uuid::new(), node_type, url.clone()));
        let _ = realm_server.notify(RealmNotification::NodeAdded((entry.0, entry.1, entry.2.clone()))).await;

        if 
            matches!(entry.1, NodeType::FrontendNode) &&
            let Ok(endpoint) = url.parse()
        {
        ...
        tokio::spawn(async move {
            while let Ok(event) = receiver.recv().await {
                if let ClusterEvent::Disconnected(peer_identity) = event {
                    ...
                    if let Some((id, node_type, url)) = state.nodes.remove(&peer_identity) {
                        let _ = realm_server.notify(RealmNotification::NodeRemoved(id)).await;

                    if 
                        matches!(node_type, NodeType::FrontendNode) &&
                        let Ok(endpoint) = url.parse()
                    {
