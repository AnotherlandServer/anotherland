...
fn generate_field_parsers(class: Rc<RefCell<ParamClass>>, _: &HashMap<String, Rc<RefCell<ParamClass>>>) -> io::Result<Vec<TokenStream>> {
    ...
    let fields = class.paramid.iter().map(|v| {
        ...
        quote! { #field_idx => context(#field_name, tuple((success(#field_idx), CParam::from_bytes)))(i), }
    }).collect();
...
pub fn generate_param_code(client_path: &Path) -> io::Result<()> {
            ...
            impl #struct_name {
                ...
                pub fn from_bytes<'a>(i: &'a [u8]) -> IResult<&'a [u8], #struct_name, VerboseError<&'a [u8]>> { 
                    context(#class_name, |i| {
                        let (i, _) = context("FirstByte", number::complete::le_u8)(i)?;
                        let (i, field_count) = context("AttributeCount", number::complete::le_u16)(i)?;
                        let (i, tagged_fields) = context("Attributes", count(|i| {
                            let (i, id) = number::complete::le_u16(i)?;
                            match id {
                                #(#field_parser)*
                                _ => context("UnknownField", fail::<&'a [u8], (u16, CParam), VerboseError<&'a [u8]>>)(i),
                            }
                        }, field_count as usize))(i)?;

                        /*if field_count as usize != tagged_fields.len() {
                            warn!("Expected {} fields, got {} for class {}", field_count, tagged_fields.len(), #class_name);
                            panic!();
                        }*/
    ...
    let struct_enum_entries: Vec<_> = paramlist.classes.iter().map(|v| {
        let class_name = v.borrow().name.to_owned();
        let struct_name = format_ident!("CParamClass_{}", class_name);

        quote! { #struct_name(Box<#struct_name>), }
    }).collect();

    let struct_enum_from_bytes: Vec<_> = paramlist.classes.iter().map(|v| {
        let class_name = v.borrow().name.to_owned();
        let struct_name = format_ident!("CParamClass_{}", class_name);
        let unique_id = v.borrow().unique_id;

        quote! { #unique_id => #struct_name::from_bytes(i).map(|(i,v)| (i, CParamClass::#struct_name(Box::new(v)))), }
    }).collect();

    let struct_enum_to_bytes: Vec<_> = paramlist.classes.iter().map(|v| {
        let class_name = v.borrow().name.to_owned();
        let struct_name = format_ident!("CParamClass_{}", class_name);

        quote! { CParamClass::#struct_name(cparam_class) => cparam_class.to_bytes(), }
    }).collect();
    ...
    write_source(&out_dir_path.join("generated_params.rs"), quote! {
        ...
        pub enum CParamClass {
            #(#struct_enum_entries)*
        }

        impl CParamClass {
            #[allow(dead_code)]
            pub fn parse_class<'a>(idx: u16, i: &'a [u8]) -> IResult<&'a [u8], CParamClass, VerboseError<&'a [u8]>> { 
                match idx {
                    #(#struct_enum_from_bytes)*
                    _ => panic!("Invalid class idx"),
                }
            }
